...
| 更新日 | 2026-06-30 |
| 状態 | 正式 |
| 関連文書 | [システムアーキテクチャ](01_システムアーキテクチャ.md), [API設計](../03_api/01_API設計.md), [ビルド実行環境設計](../07_build-runtime/01_ビルド実行環境設計.md) |

## AI Summary

ブラウザのコードをサーバーサイドでFlutter Webビルドして返す方式。IndexedDB→POST /api/builds→builderコンテナ（隔離ワークスペース）→成果物→iframeプレビューの流れ。ビルドステータスはqueued/running/succeeded/failed/timeout/cancelledの6状態。
